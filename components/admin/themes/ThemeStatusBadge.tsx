...
export function ThemeStatusBadge({
  ...
  const isPublished = !hasDraft || themesMatch(draftTheme, publishedTheme);

  const label = !hasDraft && !hasPublished ? "No theme" : isPublished ? "Published" : "Draft";

  const color = !hasDraft && !hasPublished
