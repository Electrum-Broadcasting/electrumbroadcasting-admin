...
export async function PATCH(req: Request) {
  try {
    const { cityId, accessibility } = await req.json();
    ...
    await logAdminAction(supabase, {
      ...
      action: "update_city_brand_accessibility",
      domain: "city",
    ...
    console.error("brand/accessibility PATCH error:", error);
    return NextResponse.json({ error: "Failed to update accessibility" }, { status: 500 });
  }
}
