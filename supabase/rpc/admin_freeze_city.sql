CREATE OR REPLACE FUNCTION public.admin_freeze_city(
    city_id uuid,
...
BEGIN
    ...
    UPDATE public.cities SET status = 'frozen', updated_by = auth.uid(), updated_at = now() WHERE id = city_id;
    ...
    VALUES (v_admin_id, 'freeze_city', 'city', city_id::text, city_id, COALESCE(metadata, jsonb_build_object('city_id', city_id)));
END;
$function$;