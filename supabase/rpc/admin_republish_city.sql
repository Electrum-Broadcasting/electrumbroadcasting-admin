CREATE OR REPLACE FUNCTION public.admin_republish_city(
    city_id uuid,
...
BEGIN
    ...
    UPDATE public.cities SET status = 'published', updated_by = auth.uid(), updated_at = now() WHERE id = city_id;
    ...
    VALUES (v_admin_id, 'republish_city', 'city', city_id::text, city_id, COALESCE(metadata, jsonb_build_object('city_id', city_id)));
END;
$function$;